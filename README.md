...
| 旧时代智械·鬼武士（ghost samurai） | 守门人，守在地下与星空之间 |
...
- **管道噩梦**（pipe_nightmare_*）：旧时代(20xx)的地下管道网络，也就是今天大连的地底。基地最深处连着这里，埋着旧时代的「古董」——废弃机器人（和她同源、没被选中的废品）、旧时代智械·鬼武士（旧时代守卫）、失败之作（失控被封的禁忌原型）。
- **上升路线**（pipe_ascent_*）：穿过旧时代智械·鬼武士把守的通道一路向上。失败之作把守着通往地表的路，只能用电磁脉冲「解除」而非「杀死」。
...
### 难度系统
- 新游戏开始时，在开场字幕前选择难度：**探索难度** / **标准难度**
- **探索难度**：敌人按原始数值（血量 ×1.0、伤害 ×1.0），面向轻度玩家
- **标准难度**：敌人血量 ×1.5、伤害 ×3.0，面向老玩家
- 难度写入存档，读档后保持

### 敌人
| 敌人 | 位置 | 类型 | 特色 |
|---|---|---|---|
| 机凯种·变量 | snow_1_2 | 普通 | laser/cube |
| 机凯种·常量 | base_2 | 普通 | 激光网络 |
| 机凯种·义军(女) | base_3 | 普通 | 护盾/激光/粒子 |
| 机凯种·义军(男) | base_4 | 普通 | 激光网络 / 弹簧振子 |
| 义军头目 | base_5 | BOSS | 雾门封锁 |
| 机凯种·变量（暴走）×2 | pipe_nightmare_1 | 普通 | 激光/跳跃 |
| 废弃机器人(Ⅰ型) | pipe_nightmare_1_3, 2_3 | 追逐型 | AI追玩家 |
| 旧时代智械·鬼武士 | pipe_nightmare_2_2 | BOSS | 黑暗弹/火焰墙/重力跳 |
| 失败之作 | pipe_nightmare_3_3 → 旧时代智械·鬼武士死后转移到 pipe_ascent_3 | BOSS | 噪音机制 + 秒杀（需电磁脉冲瓦解） |
| 废弃机器人(Ⅱ型) | pipe_nightmare_3_1 | 普通 | 废料传送带切轨 / 重力摆锤 |
| 斯普特尼克 | pipe_nightmare_3_1 | 普通 | 牵引光束（重力列） |
...
致敬 UT：满级 **LV 20**，三条路线（和平线 / 中立线 / 屠杀线）决定最终等级。

### 等级与经验曲线
- 满级 20：`entities/player.py` 的 `gain_exp` 用 `while self.level < 20`。
- 经验递增（致敬 UT）：升到下一级所需 = `10 × 当前等级`（LV1→2=10，LV2→3=20，…，LV19→20=190）。
- 累计到 N 级 = `10 + 20 + … + 10×(N-1)` = `5×N×(N-1)`：LV10 = 450，LV19 = 1710，LV20 = 1900。
- 升级奖励：每级 `+3 HP / +2 攻击`（`level_up`；满级 LV20 = **HP 77 / ATK 48**，克制容错、避免秒杀）。

### 敌人 HP（数值大换血后）
按「教学 → 普通 → 精英 → boss」递进，初始 ATK 10 起算刀数（满级 ATK 48）：

| 敌人 | HP | 初始刀数 | 满级刀数 |
|---|---|---|---|
| 机凯种·变量（雪地） | 30 | 3 | 1 |
| 机凯种·变量（暴走×2） | 50 | 5 | 2 |
| 机凯种·常量（base_2） | 40 | 4 | 1 |
| 机凯种·义军(女)（base_3） | 70 | 7 | 2 |
| 机凯种·义军(男)（base_4） | 70 | 7 | 2 |
| 义军头目（base_5 boss） | 140 | 14 | 3 |
| 废弃机器人(Ⅰ型) | 50 | 5 | 2 |
| 废弃机器人(Ⅱ型) | 80 | 8 | 2 |
| 斯普特尼克 | 60 | 6 | 2 |
| 旧时代智械·鬼武士 | 130 | 13 | 3 |
...
### 三条路线（目标）
- **和平线**：一个不杀 → 停在 LV 1。
- **中立线**：每个敌人全砍一遍 → 目标 **LV 19**（全游戏口径，待基地章节做完后精确定值）。
- **屠杀线**：每个小怪坐火复活、最多杀 5 次后彻底消失 → 目标 **LV 20（满级）**。
  - 复战（第 2~5 次击杀）固定经验，值待定（候选 10）。
  - 19→20 门槛 = 屠杀线复战总经验，**待游戏做完、加完小怪后再定**。

### 已定的经验表（管道噩梦及之前 → LV 10）
snow + 管道噩梦 10 场全砍一遍，合计正好 **450 = LV 10**：
...
### 待定（游戏做完再定）
- 基地章节（`base_1~5`：机凯种·常量、机凯种·义军(女)、机凯种·义军(男)、义军头目）的经验，负责 10 级以后的升级（当前仍为旧值 5/10/5 占位）。
- 19→20 门槛 + 屠杀线复战机制：坐火复活已有（`temp_killed_enemies` 休息/死亡后重置），但「最多杀 5 次→彻底消失」与「复战固定经验」尚未实装。
...
- **星海广场特殊篝火 + 解析系统**：`star_sea_plaza` 新增特殊篝火，首次触碰弹出提示；解锁后所有篝火的休息菜单都会出现「解析」功能。解析会筛选带 `boss_trophy` 特殊 tag 的 boss 战利品（义军头目「黑色游侠的动力炉」、旧时代智械·鬼武士「鬼武士的动力炉」），选中后进入强化二选一（强化 A/B 的具体效果待后续实装）。
...
### 难度系统
- **难度选择**：新游戏开场字幕前新增难度选择——**探索难度**（敌人原始数值，面向轻度玩家）与**标准难度**（敌人血量 ×1.5、伤害 ×3.0，面向老玩家）。
- **难度随档**：难度写入存档，读档后保持。

### 男性义军技能组重做
- **技能轮换**：男性义军（机凯种·义军(男)）从单技能「激光网格」改为两技能组交替——奇数回合激光网格、偶数回合**弹簧振子**。
- **弹簧振子**：战斗框缩成横置窄条（高 64px），红心锁定中线沿水平做简谐振动（回复力 + 左右键外力 + 阻尼）；红心左右两端各一条像素弹簧随振动伸缩；屏幕上下竖直飞来激光束（复用激光网格的 LaserNetworkLine）；回合时长 +3s，结束后恢复战斗框大小。
- **生命值**：男性义军 HP 55 → 70，与女性义军一致。

### 其他
- **投掷电池**：伤害每级补正 +1 点。
- **归航信标**：传送菜单改为两列布局，修复第四个篝火超出屏幕的问题。
