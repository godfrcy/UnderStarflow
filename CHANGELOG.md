# 更新日志

## [0.5.0] - 2026-08-16

### 难度系统
- 新游戏开场字幕前新增难度选择：探索难度（敌人原始数值）/ 标准难度（敌人血量 ×1.5、伤害 ×3.0）
- 难度写入存档，读档后保持

### 数值与等级大换血
- 满级 LV 20，经验需求随等级递增（升下一级 = 10 × 当前等级）
- 升级奖励 HP +3 / 攻击 +2（满级 HP 77 / ATK 48）

### 男性义军技能组重做
- 两技能组交替：奇数回合激光网格、偶数回合弹簧振子
- 弹簧振子：横置窄条场地、水平简谐振动、左右像素弹簧、上下竖直飞来的激光束
- 男性义军 HP 55 → 70

### 修复与优化
- 投掷电池伤害每级补正 +1
- 归航信标传送菜单两列换行，修复第四个篝火出屏
- 资源整理：删除 300 余个未引用资源文件、中文文件名改英文、修复音乐空格引用

## [0.4.0] - 2026-08-14

### 上升管道路线 🚀
- 击败鬼武士后向左进入通往地表的三张连续管道图 `pipe_ascent_1 → pipe_ascent_2 → pipe_ascent_3`，复用管道噩梦1-2横向管道素材
- `pipe_ascent_2`（阴湿管道）屏幕正中间新增篝火存档点
- `pipe_ascent_1` 放置「电磁脉冲」光点（id=`emp_pulse`）

### 失败之作机制 👻
- 鬼武士死亡后，失败之作从 `pipe_nightmare_3_3` 消失，转移到 `pipe_ascent_3` 最左侧
- 此战玩家必先手，使用电磁脉冲永久解除失败之作的秒杀机制（写入存档，读档不失效）
- 失败之作在上升管道3中只显示「向右移动」贴图，缓慢向右漂移；玩家距离 ≤90px 时转为正常追逐

### 其他 🧪
- 新增测试道具「测试」，开局入包，战斗造成 1000 伤害且不消耗，便于测试路线

## [0.3.0] - 2026-07-23

### 弹幕设计器 🎯
- 新增 `bullet_designer.py` 可视化弹幕设计器
  - 与游戏战斗区域等大（400×300）的画布 + 网格覆盖
  - 时间线编辑系统（逐帧控制弹幕生成时机）
  - 8 种弹幕类型：普通子弹、延时激光、方块、圆环、等离子刃、激光网、黄线、追踪弹、扇形散射
  - 实时预览功能（方向键控制灵魂躲避）
  - 属性面板快速编辑弹幕参数
  - 导出 JSON 到 `assetsDB/patterns/`
- 新增 `engine/pattern_loader.py` JSON 模式加载器
  - 数学表达式求值（sin/cos/random）支持复杂弹幕轨迹
  - PatternRunner 在战斗中按时间线执行弹幕生成
- 战斗系统集成：敌人技能列表中使用 `@pattern_name` 引用 JSON 模式
- 3 个示例弹幕：`simple_rain`、`laser_maze`、`spiral_hell`

### 启动方式
```bash
python bullet_designer.py   # 启动弹幕设计器
```

## [0.2.0] - 2026-07-23

### 项目整理
- 🧹 删除 12 个无用 zip 压缩包
- 🗑️ 删除遗留 `maps/` 目录（已迁移到 `assetsDB/maps/`）
- 🗑️ 删除一次性脚本 `organize_assets.py`、`verify_ghost.py`
- 📁 30+ 个散落在 `assetsDB/` 根目录的文件归类到 `characters/`、`objects/`、`ui/` 子目录
- 🏷️ 所有中文目录重命名为英文（`失败之作` → `failure_boss`、`废弃机器人` → `abandoned_robot` 等）
- 🎵 音频文件标准化命名（去空格、去中文、统一小写）
- 🔧 更新所有代码中的资源路径引用

## [0.1.0] - 2026-07-23

### 新增
- 初始化项目，提交初始版本代码
- 添加 `README.md` 项目说明文档
- 添加 `CHANGELOG.md` 更新日志

### 基础设施
- 配置 Git 远程仓库：https://github.com/godfrcy/UnderStarflow
- 配置 SSH Key 完成 GitHub 认证
- 首次推送，项目正式开源

## [0.0.1] - 初始版本

- 🎮 Pygame 2D 俯视角弹幕躲避 RPG
- 瓦片地图系统（雪地/基地/管道噩梦）
- 实时弹幕战斗系统（普通/激光/方块/等离子刃/激光网络）
- 魂系篝火存档机制
- 完整 UI 系统（标题/暂停/篝火/传送/背包菜单）
- 对话系统与多种特效
- PyInstaller 打包支持
